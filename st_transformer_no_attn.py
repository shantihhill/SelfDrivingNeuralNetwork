best_model = 'encoder_mlp_velocity_attn'
import os
...
class AgentEncoder(nn.Module):
    def __init__(self, input_dim, d_model, max_len=50, n_heads=8, num_layers=2):
        super().__init__()
        self.input_proj = nn.Linear(input_dim, d_model)
...
def train_and_evaluate_model(device):
    # Create model
    model = VehicleTrajectoryPredictor(input_dim = 5, output_dim = 60*2 , d_model = 256)
...
#     model = VehicleTrajectoryPredictor(input_dim = 5, output_dim = 60*2 , d_model = 256).to(device)
...
#     output_df.to_csv('submission_encoder_mlp_v1.csv', index=True)
...
def predict_model(device):
    ...
    best_model2 = torch.load(os.path.join("models", best_model+str(85)+".pth"))
    model = VehicleTrajectoryPredictor(input_dim = 5, output_dim = 60*2 , d_model = 256).to(device)
    # optimizer = optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4)
    # scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.25) # You can try different schedulers
    # criterion = nn.MSELoss()
    ...
    output_df = pd.DataFrame(pred_output, columns=['x', 'y'])
    output_df.index.name = 'index'
    output_df.to_csv('submission_transformer_attn_v1.csv', index=True)
